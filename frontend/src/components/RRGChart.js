...
import { getSectorsRRG, getThemeRRG, getThemes, getThemeRRGBaskets } from '../api/client';
...
const CustomTooltip = ({ active, payload }) => {
  ...
  const sectorName = SECTOR_NAMES[d.ticker];
  ...
  return (
    <g>
      {chartData.map((item, idx) => {
        ...
        // Dim non-hovered trails when something is hovered
        const trailOpacityBase = hasHover ? (isHovered ? 0.5 : 0.06) : 0.3;
        const trailOpacityEnd = hasHover ? (isHovered ? 0.9 : 0.1) : 0.7;
        const dotOpacityBase = hasHover ? (isHovered ? 0.6 : 0.08) : 0.35;
        const dotOpacityEnd = hasHover ? (isHovered ? 1.0 : 0.12) : 0.8;
...
const RRGChart = () => {
  ...
  const visibleSectors = (showSectors || viewMode !== 'sectors') ? chartData : [];
  const visibleBaskets = (showBaskets && viewMode === 'sectors') ? basketData : [];
  const allData = [...visibleSectors, ...visibleBaskets];

  // For axis range, always include all data (even hidden) so toggling doesn't jump
  ...
  const pad = 2;
  const xMin = allR.length ? Math.min(Math.floor(Math.min(...allR) - pad), CENTER - 10) : CENTER - 15;
  const xMax = allR.length ? Math.max(Math.ceil(Math.max(...allR) + pad), CENTER + 10) : CENTER + 15;
  const yMin = allM.length ? Math.min(Math.floor(Math.min(...allM) - pad), CENTER - 10) : CENTER - 15;
  const yMax = allM.length ? Math.max(Math.ceil(Math.max(...allM) + pad), CENTER + 10) : CENTER + 15;

  // Add color to each item
  const sectorCount = visibleSectors.length;
    ...
    return (
      ...
      >
        <circle cx={cx} cy={cy} r={isHovered ? 9 : 7}
          ...
          stroke={isHovered ? '#fff' : '#fff'} strokeWidth={isHovered ? 2 : 1.5} />
        <text x={cx} y={cy - 13} textAnchor="middle" fill="#fff"
          fontSize={10} fontWeight="bold" opacity={dimmed ? 0.2 : 1}
        >
          {payload.ticker}
        </text>
        {isHovered && SECTOR_NAMES[payload.ticker] && (
          <text x={cx} y={cy + 20} textAnchor="middle" fill="#aaa"
            fontSize={9} fontWeight="normal"
          >
            {SECTOR_NAMES[payload.ticker]}
          </text>
        )}
      </g>
    );
  ...
  return (
    <div style={{
        ...
        <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
          {viewMode === 'sectors' && (
            <>
              ...
              <label style={{ display: 'flex', alignItems: 'center', gap: '4px', color: '#888', fontSize: '11px', cursor: 'pointer' }}>
                <input
                  type="checkbox" checked={showSectors}
                  onChange={(e) => setShowSectors(e.target.checked)}
                  style={{ accentColor: '#00ff88' }}
                />
                Sectors
                <InfoTooltip text={METHODOLOGY_TEXT.sectors} />
              </label>
            </>
          )}
      ...
      {/* Legend */}
      <div style={{ display: 'flex', gap: '24px', marginTop: '12px', fontSize: '12px', color: '#888', flexWrap: 'wrap' }}>
        {[
        ...
        {showBaskets && viewMode === 'sectors' && basketData.length > 0 && (
